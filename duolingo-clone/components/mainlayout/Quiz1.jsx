...
export default function Quiz({ quizSlug = 'quiz1' }) {
  ...
  useEffect(() => {
    async function fetchQuiz() {
      try {
        const res = await fetch(`http://localhost:5000/api/quizzes/${quizSlug}`);
  ...
  // Assuming the correct answer is marked in the data or default to index 1
  ...
  const handleCheckOrNext = () => {
    if (!checked) {
      ...
      // route to next quiz or page; customize as needed
      router.push('/quiz2');
    }
  };
