...
export default function Quiz() {
  ...
  useEffect(() => {
    async function fetchQuiz() {
      try {
        const res = await fetch('http://localhost:5000/api/quizzes/quiz2');
  ...
  // Assuming correct answer is always at index 1 (second option)
  ...
  const handleCheckOrNext = () => {
    if (!checked) {
      ...
      router.push('/quiz3'); // navigate to next quiz
    }
  };
